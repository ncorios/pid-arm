...
# simple experiment to test the PID controller on the arm. We will apply a step input in desired angle and see how the arm responds under PI control.
# k_p will be kept from last experiment to reflect steady state error, and the failure of linear approximation.
# k_i will be kept from the last experiment to elminate steady state error, but it leaves oscillations
# k_d will be added to dampen the oscillations. my value is very high to show the effect of the D term
# you should tune the gains yourself. these are set to see the effect of each term, not to be optimal
...
if __name__ == "__main__":
    ...
    save_run('outputs/arrays/pid.npz', t_arr, states, np.array(taus), theta_d, arm, pid) # save data for later analysis
